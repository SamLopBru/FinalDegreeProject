...
#modify block --> the class was changed because randperm works with int64 and is not valid in TPU
class BalancedPositiveNegativeSampler:
    ...
    def random_selection(self, tensor: Tensor, num_samples: int) -> Tensor:
        ...
        if tensor.numel() > 0 and num_samples > 0:
            ...
            _, indices = torch.topk(rand_scores,  #topk is more compatible than argsot in TPU
                                  k=min(num_samples, tensor.numel()))
            return tensor[indices.to(torch.int32)]
        return tensor[:0]  # returns an empty tensor if there are no elements to select
...
#modified block --> because the substraction of the boxes is 0, it caused -inf or nan in the loss, so a minimum value has been set
